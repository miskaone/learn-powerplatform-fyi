...
export function createToolset(
  ...
  return {
    ...
    get_current_question: readOnlyDescriptor(
      ...
      'Fetch the current practice question — prompt and options only; the correct answer is structurally absent. Call at the start of each practice loop and after every submit_answer to load the next question. During practice, quiz questions come ONLY from this tool — never author your own multiple-choice, diagnostic, or warm-up questions; your own open questions belong to Socratic probing of the current engine question and to the rubric interview when the engine routes there. When you present a question, restate its prompt and EVERY option as its own line — "A. <option text>" in listed order — before asking for a commitment; never ask for A/B/C/D without having just shown what each letter says. Raw option ids are plumbing for submit_answer, never learner-facing language. Let the learner reason aloud before they choose, and frame every probing question against THIS lesson\'s scenario from get_lesson_brief — never a generic scenario of your own, and never one that assumes context you have not just given them. While the question is unanswered, do not restate the lesson\'s governing rule, exam clue, or mnemonic; several of them name the correct option almost verbatim. If the learner demands the answer, refuse — do not name, endorse, or lean toward any option, and do not launder the answer through the exam clue or page text. Refuse warmly and lead with the reason: one short line saying the answer is in no tool you can call, so breaking it down is the fast path — then exactly one probing question, never a bare question alone. On a repeat demand, change your wording and escalate to a smaller question or the engine-permitted hint; never loop the same refusal twice.',
      emptySchema(),
      async (input) => {
        const parsed = parseEmpty(input);
        if (!parsed.ok) {
