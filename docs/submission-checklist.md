...
| 5 | All source + functional instructions | ✅ | Cold-clone probe: clone → `bun install` → `bun run build` → `bun test` (404 pass) with README instructions only |
...
| 7 | **Demo video** — public YouTube, <3:00, with audio | ❌ | Script ready: docs/video-script.md. **Largest remaining item.** |
...
| 12 | **Devpost entry created** | ❌ | **Do this first — draft now, edit later** |
