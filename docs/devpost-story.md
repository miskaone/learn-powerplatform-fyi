# Devpost Story — Mastery Gate (final, 2026-09-02)
...
tools, so the site decides what the AI may know, say, and be. I'd already built
this pedagogy once without AI — deterministic scoring, named misconceptions,
repair routes — for my daughter's engineering statics course. Mastery Gate is
what that pattern becomes when you hand it to an agent.
...
- A deterministic TypeScript engine grades, counts attempts, diagnoses **named
  ...
  Transfer, every dimension ≥ 3 — never an average), and gates progression.
...
- **Capability is the guardrail:** `advance_module` does not exist until the
  ...
  opens, 2 during the exam** — the site's decisions, visible to learner and
  agent alike.
...
- `getTools()`/`registerTool()` return Promises in real runtimes; our lenient
  mocks hid that until production crashed. Chrome also enforces
...
- **The subtlest bug was pedagogical.** Early on we redacted so hard the coach
  ...
  page gets — no more, no less.** Redaction is symmetry, not secrecy.
- **The host moved under us (Aug 28 → Sep 2).** The ChatGPT desktop app's
...
- We surveyed five agent surfaces and found only two can invoke page tools
  today (ChatGPT's desktop browser, Chrome 152 + flag); three read but cannot
  call. The ecosystem gap is real — see What's next.
...
- The site reacts to its own state instead of asking the agent to: a miss
  ...
  roster, `start_exam` mounts the exam where the learner is standing — the page
  moves, the chat stays put. And when the interview isn't unlocked yet, the
  referee says *why*: `request_next_action` returns the per-dimension coverage
  ledger, so "not yet" always comes with "here's what's missing."
- A **Tool Inspector** on the page: schema-generated forms for every registered
  tool, so anyone in any browser can invoke the real surface — same guards,
  same redaction — with no agent at all.
...
- **The bridge** (in-repo companion, tested; live pairing is roadmap): an MV3
  extension + local MCP server so *any* MCP-speaking client — CLI agents
